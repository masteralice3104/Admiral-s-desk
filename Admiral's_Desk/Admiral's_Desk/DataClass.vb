...
Public Class StructureOperationClass
    ...
    'これは1回しか起こらないはず
    '無論更新した場合は除く
    Public Sub KancolleCommonDataReset(oSession As Nekoxy.Session)
        ...
        If Component.KancolleReadJson(oSession, URLDataClass.api_start2) IsNot Nothing Then
            ...
            '艦娘数を元に配列の調整
            ReDim CommonDataClass.AllKanmusuData(JsonData("api_data")("api_mst_ship".Count))

            'AllKanmusuDataはDataclass.CommonData.Kanmusu型なので個々の要素の配列を調整する必要がある
            'よって必要な配列の調整を行う
            For Each KanmusuCount As DataClass.CommonData.Kanmusu In CommonDataClass.AllKanmusuData
    ...
    'Jsonのデータを構造体にぶち込む
    Public Sub JsonDataInputToStructure(ByVal JsonObject As Newtonsoft.Json.Linq.JObject)
